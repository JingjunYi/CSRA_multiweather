...
def get_label(data_path):
    print("generating labels for multiweahter dataset")
    xml_paths = os.path.join(data_path, "Annotations/")
    save_dir = "E:/DeepLearning/multiweather/supervised/CSRA-master/data/multiweather/labels"
    ...
    for i in os.listdir(xml_paths):
        ...
        for obj in obj_all:
            ...
            cls.append(category_info[one_class])
...
def transdifi(data_path):
    print("generating final json file for multiweahter dataset")
    label_dir = "E:/DeepLearning/multiweather/supervised/CSRA-master/data/multiweather/labels"
    img_dir = os.path.join(data_path, "JPEGImages/")

    # get trainval test id
    id_dirs = os.path.join(data_path, "ImageSets/Main/")
    f_trainval = open(os.path.join(id_dirs, "trainval.txt"), "r").readlines()
    # f_val = open(os.path.join(id_dirs, "val.txt"), "r").readlines()
    # f_trainval = f_train + f_val
    f_test = open(os.path.join(id_dirs, "test.txt"), "r")

    trainval_id = np.sort([int(line.strip().split('_')[-1]) for line in f_trainval]).tolist()
    test_id = [int(line.strip().split('_')[-1]) for line in f_test]
    trainval_data = []
    test_data = []
    ...
    for item in sorted(os.listdir(label_dir)):
        with open(os.path.join(label_dir, item), "r") as f:
            ...
            if int(item.split('.')[0].split('_')[-1]) in trainval_id:
            ...
            if int(item.split('.')[0].split('_')[-1]) in test_id:
                data = {"target": target.tolist(), "img_path": img_path}      
                test_data.append(data)

    json.dump(trainval_data, open("data/multiweather/trainval_multiweather.json", "w"))
    json.dump(test_data, open("data/multiweather/test_multiweather.json", "w"))
    print("multiweather data preparing finished!")
    print("data/multiweather/trainval_multiweather.json data/multiweather/test_multiweather.json")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", default="E:/DeepLearning/multiweather/dataset/MultiWeather/VOC", type=str, help="The absolute path of VOCdevkit")
    args = parser.parse_args()
    
    if not os.path.exists("E:/DeepLearning/multiweather/supervised/CSRA-master/data/multiweather"):
        os.makedirs("E:/DeepLearning/multiweather/supervised/CSRA-master/data/multiweather")
